Neural AI systems are often described as "black boxes", complex machines that generate human-like responses while their inner workings remain out of reach of human understanding. Therefore, a new scientific program has emerged that treats AI systems as objects of study, aiming to discover general laws and construct coherent explanatory frameworks. This field, known as **Mechanistic Interpretability (MI)**, can be thought of as the neuroscience of AI—an attempt to reverse-engineer neural networks by carefully observing and manipulating their computations to find human-understandable rules governing their behavior.

Then, an interesting question arises: Are the explanations sought by MI guaranteed to be _unique_? If not, what should we do if different, mutually exclusive _explanations_ coexist?
...
However, our concern is different: we are touching on the problem of identifiability. Identifiability states that if we fix the rules of the game—level of abstraction, query, dataset, and explanatory framework—there should be only one valid explanation (up to trivial isomorphisms). Our findings provide strong empirical evidence that, within current frameworks, mechanistic explanations are not unique. What does this mean for the goals and definitions of MI?

---

### Identifiability

In science, identifiability ensures that when we observe something, we can infer a single, unique explanation from the data. One standard example is the Plato's cave thought experiment. Imagine standing in a dimly lit cave, trying to determine an object’s shape from its shadow. If different objects can cast the same shadow, our observations fail to uniquely determine reality. In statistics, a model is said to be *identifiable* if its parameters can be uniquely inferred from data, preventing such ambiguity.

However, identifiability is not an inherent property of the world, instead, it is a property of the models we use to explain it. Many scientific fields require identifiability to draw reliable conclusions. Without it, multiple mutually exclusive explanations can exist for the same phenomenon, leading to confusion rather than clarity. In general, identifiability is of great concern to statisticians as it is a prerequisite to perform sound statistical and causal inference.
...
A full mechanistic explanation consists of two components:
...
Together, these form a computational explanation—a lossy compression of the network’s complex computations into a simpler algorithmic form. The identifiability question then becomes: If we fix the behavior of interest (a set of inputs) and the class of allowed explanatory models, does the network’s computation uniquely determine the explanation? Of course, it depends on what counts as an _explanation_. MI has different answers to this and we explore several of them.

#### Mechanistic Interpretability: Strategies

Before addressing identifiability, it is important to recognize that MI is an evolving field with diverse methodologies. 
We find two main broad strategies that seek to bridge low-level neural activations to interpretable algorithms:
...
2. **Explanation Trustworthiness**:
   Non-unique explanations risk being perceived as *post hoc rationalizations* rather than ground truth. If multiple algorithms align equally well with a circuit and no meaningful heuristic can distinguish them, which one should we trust/choose as our metaphor for "understanding"?
